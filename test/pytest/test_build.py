...
def test_build_x86(out_stream: MyStream, cli: JabberwockyCLI) -> None:
    ...
    send_cmd_to_cli(cli, out_stream, ['install', 'x86_building/build/jcontainer.tar.gz', 'x86_built'])
    send_cmd_to_cli(cli, out_stream, ['start', 'x86_built'])
    try:
        send_cmd_to_cli(cli, out_stream, ['run', 'x86_built', 'gdb', '-o', 'hello.o', '/share/hello_world.c'])
        s = send_cmd_to_cli(cli, out_stream, ['run', 'x86_built', './hello.o'])
        assert s == 'Hello World\n'
        s = send_cmd_to_cli(cli, out_stream, ['run', 'x86_built', 'python', 'hello_world.py'])
        assert s == 'Hello World\n'
    finally:
...
def test_build_arm(out_stream: MyStream, cli: JabberwockyCLI) -> None:
    ...
    send_cmd_to_cli(cli, out_stream, ['install', 'arm_building/build/jcontainer.tar.gz', 'arm_built'])
...
def test_build_mips(out_stream: MyStream, cli: JabberwockyCLI) -> None:
    ...
    send_cmd_to_cli(cli, out_stream, ['install', 'mips_building/build/jcontainer.tar.gz', 'mips_built'])
